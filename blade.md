...
Blade adalah *templating engine* Laravel yang sederhana tapi powerful. Tidak seperti *templating engine* PHP populer yang lain, Blade tidak membatasimu jika ingin menggunakan kode PHP murni di dalam view. Bahkan, semua view yang ditulis dengan Blade dikompilasi ke dalam bentuk PHP murni lalu di cache sampai dimodifikasi, artinya Blade tidak menambah kompleksitas apapun ke aplikasi yang dibangun. File view Blade menggunakan akhiran `.blade.php` dan umumnya disimpan di dalam direktori `resources/views`.
